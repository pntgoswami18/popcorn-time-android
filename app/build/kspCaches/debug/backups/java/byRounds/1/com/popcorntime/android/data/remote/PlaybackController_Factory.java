package com.popcorntime.android.data.remote;

import dagger.internal.DaggerGenerated;
import dagger.internal.Factory;
import dagger.internal.QualifierMetadata;
import dagger.internal.ScopeMetadata;
import javax.annotation.processing.Generated;

@ScopeMetadata("javax.inject.Singleton")
@QualifierMetadata
@DaggerGenerated
@Generated(
    value = "dagger.internal.codegen.ComponentProcessor",
    comments = "https://dagger.dev"
)
@SuppressWarnings({
    "unchecked",
    "rawtypes",
    "KotlinInternal",
    "KotlinInternalInJava",
    "cast"
})
public final class PlaybackController_Factory implements Factory<PlaybackController> {
  @Override
  public PlaybackController get() {
    return newInstance();
  }

  public static PlaybackController_Factory create() {
    return InstanceHolder.INSTANCE;
  }

  public static PlaybackController newInstance() {
    return new PlaybackController();
  }

  private static final class InstanceHolder {
    private static final PlaybackController_Factory INSTANCE = new PlaybackController_Factory();
  }
}
